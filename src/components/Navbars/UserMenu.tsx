...
import {ChevronDown, ChevronRight, ChevronUp} from 'baseui/icon'
        ...
        overrides={{
          LabelContent: {
            style: ({$theme}) => ({
              color:
                $theme.colors[
                  props.$isHighlighted ? 'primaryB' : 'contentPrimary'
                ],
            }),
          },
        }}
...
const UserInfoBlock = ({username, usernameSubtitle}) => (
  <Block padding='0px 5px' display='inline-block'>
    <UserInfoLabel font='Label2' color='primaryB'>
      {username}
    </UserInfoLabel>
    <UserInfoLabel font='Label3' color='contentTertiary'>
      {usernameSubtitle}
    </UserInfoLabel>
  </Block>
)
